"""
Clean Enhanced Contract Parser
...
class EnhancedContractParser:
    """Clean, efficient contract parser using Direct Gemini AI"""
    
    def __init__(self):
        self.direct_gemini_extractor = DirectGeminiExtractor()
    
    async def parse_contract(self, file_path: str) -> ContractData:
        """Parse contract using Direct Gemini AI"""
        try:
            logger.info("Starting direct Gemini contract extraction")
            return await self.direct_gemini_extractor.extract_contract_data(file_path)
            
        except Exception as e:
            logger.error(f"Direct Gemini extraction failed: {str(e)}")
            return self._get_fallback_data()
    
    def _get_fallback_data(self) -> ContractData:
        """Return fallback data when extraction fails"""
        from .models import (SLA, ContractSummary, DocumentMetadata,
                             FinancialDetails, PaymentTerms,
        ...
        return ContractData(
            ...
            compliance_issues=[],
            important_dates=[],
            processing_notes=["Extraction failed, using fallback data"],
            clauses=[],
